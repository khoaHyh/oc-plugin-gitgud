...
describe("Git change-set", () => {
  test("derives display facts from porcelain and numstat", () => {
    const files = createGitChangeSet({
      statusOutput: " M src/tui.tsx\0M  README.md\0?? scratch.txt\0",
      numstatOutput: "4\t2\tsrc/tui.tsx\0",
    })
  ...
  test("keeps rename display facts with the current path as the action path", () => {
    const files = createGitChangeSet({
      statusOutput: "R  src/new.ts\0src/old.ts\0",
      numstatOutput: "1\t1\tsrc/new.ts\0",
    })
